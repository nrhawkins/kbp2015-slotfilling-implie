...
object SingleNameResolver {
  ...
  def singleQueryName(doc: Annotation, q: KBPQuery, annotatorHelper: StanfordAnnotatorHelperMethods): (Boolean,String) = {
    
    //val names = DocUtils.stanfordHelper.getFullNamesFromCorefMentions(doc, Interval.closed(q.begOffset,q.endOffset)).asScala
    val names = annotatorHelper.getFullNamesFromCorefMentions(doc, Interval.closed(q.begOffset,q.endOffset)).asScala
