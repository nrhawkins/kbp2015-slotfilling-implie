...
object RunKBP2015Implie {
  
  val config = ConfigFactory.load("tac-runkbp2015-implie.conf")
  ...
  def main(args: Array[String]) {
    ...
    //queries.foreach(q => { 
    //  println(q.id + " " + q.name) 
    //  q.slotsToFill.foreach(s => println(s.name))
    //})
    ...
    /*var totalDocs = 0
	  for(query <- queries){
	      ...
	      outputStreamRelDocs.println(query.id + " " + numberOfDocs)
	  }
	  outputStreamRelDocs.println("Total Docs: " + totalDocs)
	  println("Total Docs: " + totalDocs)
	  val totalUniqueDocs = entityRelevantDocSerialization.values.toSet.flatten.size
	  outputStreamRelDocs.println("Total Unique Docs: " + totalUniqueDocs)
	  println("Total Unique Docs: " + totalUniqueDocs)
    ...
    //println(System.getProperty("java.util.logging.config.file"))
    //val y: InputStream = new FileInputStream("/homes/gws/nhawkins/kbp2015-slotfilling-implie/src/main/resources/logging.properties")    
    //val lm = LogManager.getLogManager
    //lm.readConfiguration(y)
    //println(System.getProperty("java.util.logging.config.file"))
    //println(System.getProperty("log4j.configurationFile"))
    //val l = lm.getLoggerNames().asScala
    //l.foreach(l => println(l))  
    //println("global level: " + lm.getLogger("global").getLevel.getName)
    //lm.getLogger("global").setLevel(Level.WARNING)
    //val gl = lm.getLogger("global").getLevel()
    //println("global level: " + gl.getName)
    
    //val doc1 = SolrHelper.getRawDoc("eng-NG-31-108068-8951031")
    //val doc2 = SolrHelper.getRawDoc("eng-NG-31-127622-9244378")
    
    //println("Doc Length: " + doc1.length)
    //println("Doc Length: " + doc2.length)
    ...
    //println("Num of Cold Start Slots: " + ColdStartSlots_ImplIE.slots.size)
    ...
    for(query <- queries){
      ...
      var singleQueryNamePER = false
        ...
        if(roundID == "round2"){
          ...
          //println("queryName: " + query.name)

          singleQueryNamePER = query.entityType match{
            case PER if(query.name.split(" ").size == 1) => {val (single, qname) = SingleNameResolver.singleQueryName(query, queryNameSetRound2, queryNameSetRound1)
          ...
          //println("snr: " + singleQueryNamePER + " " + query.name)  
      ...
      if(query.slotsToFill.size==1 && ColdStartSlots_ImplIE.slots.contains(query.slotsToFill.toList(0).name) && !singleQueryNamePER){
         ...
         //})      
          ...
          //println("Query: " + query.id + " " + query.name)
		  //println("Size All Documents: " + relevantDocs.size)         
          ...
          var totalSentences = 0
  		  ...
  		  //if(slots.size == 1){println("Slot To Fill: " + slots.toList(0).name )}
