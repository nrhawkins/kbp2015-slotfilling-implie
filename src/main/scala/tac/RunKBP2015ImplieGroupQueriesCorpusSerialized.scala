...
object RunKBP2015ImplieGroupQueriesCorpusSerialized {
  
  val config = ConfigFactory.load("tac-runkbp2015-implie-groupQueriesCorpusSerialized.conf")
  ...
  def main(args: Array[String]) {
    ...
    //val outRelDocs = new PrintStream(numRelDocsFileName)
    
    //val corpusOldNew = corpusName
    //val relevantDocsFileName = relDocsFileName
    //val relevantDocsFile = new File(relevantDocsFileName)  

    val queriesFileName = args(0)
    val corpus = args(1)
    val relevantDocsFileName = args(2)
    val relevantDocsFile = new File(relevantDocsFileName)      
    val outputStream = new PrintStream(args(3))
    val roundID = args(4)
    val batchDrop = args(5).toInt
    val batchDropRight = args(6).toInt
    val pathToSerializedCorpus = args(7)
    
    println("corpus: " + corpus)
    println("rel docs: " + relevantDocsFileName)
    println("round id: " + roundID) 
    println("drop: " + batchDrop)
    println("drop right: " + batchDropRight)
    println("ser corp: " + pathToSerializedCorpus)
    //val fullDocNameWithPath = "/projects/WebWare6/KBP_2015/corpus/serialized/NYT_ENG_20131231.0228.ann"
    //val fullDocNameWithPath = "/projects/WebWare6/KBP_2015/corpus/serialized_corenlp_3.5/00015379a05948e47bd0d26c606208e3.ann"
    //val fullDocNameWithPath = "/projects/WebWare6/KBP_2015/corpus/serialized_corenlp_3.5/ENG_NW_001278_20131110_F00011SRW.ann"
    //val fullDocNameWithPath = "/projects/WebWare6/KBP_2015/corpus/serialized_corenlp_3.5/ENG_NW_001278_20130912_F000125VV.ann"
    //val fullDocNameWithPath = "/projects/WebWare6/KBP_2015/corpus/ser_3.5_eng/ENG_NW_001278_20130912_F000125VV.ann"
    ...
    //println("sentence 0: " + docSentences(0))
    //val docTokens = docSentences(0).get(classOf[TokensAnnotation]).asScala.toList   
    //docTokens.foreach(t => println("token: " + t.originalText + " start: " + t.beginPosition))    
    
    //SolrHelper.setConfigurations(corpus, false)
    //val rawDoc = SolrHelper.getRawDoc("ENG_NW_001278_20131110_F00011SRW")
    ...
    //println("rd: queries size: " + queries.size)
    //println("rd: query name: " + queries(0).name)
    //println("rd: num aliases: " + queries(0).aliases.size)
    //println("rd: aliases: ")
    //queries(0).aliases.foreach(a => println(a))
    ...
    /*var sameQueryCount = 0
    var td = 0
    for(sameQuery <- sameQueries){

      sameQueryCount += 1
      val firstQuery = sameQuery(0)
      ...
      firstQuery.entityType match{
        case PER if firstQuery.name.split(" ").size == 1 => 
        case _ => td += relevantDocs.size
      }
      
      //td += relevantDocs.size
      outRelDocs.println(sameQueryCount + " " + firstQuery.name + " " + sameQuery.size + " " + relevantDocs.size + " " + td)
      ...
      if(!singleQueryNamePER && anyRelevantSlots){
	    ...
	    try{
		  ...
		  for(document <- relevantDocs){
		    ...
		    println("query count: " + queryCount + " doc count: " + docCount + " " + document)
		    ...
		    //println("Checking if File exists")
              ...
              if (Files.exists(Paths.get(fullDocNameWithPath))) { 
                ...
                //println("Deserializing file: " + fullDocNameWithPath)                
                
                var numDocSentences = 0
                ...
                try{
                  ...
                  numDocSentences = docSentences.size
                
                if(numDocSentences > 0){
		    
                  //println("document: " + document)
                  
		          val (sentences, corefMap) = getSentencesAndCorefMap(Some(doc))  		        
  		          val matchingCorefMentions = getMatchingCorefMentions(corefMap, firstQuery)  	
  		         
  		          val relevantSentencesCoref = getRelevantSentencesIncludingCoref(Some(doc), firstQuery, matchingCorefMentions)    
                  ...
                  totalSentences += relevantSentences.size
                    ...
                    extractions.foreach(e => {//println(e.getDocName() + " " + e.getArg1() + " " + e.getArg2())
                                              if(e.getDocName() == null) e.setDocName(document)
                                              //println(e.getDocName() + " " + e.getArg1() + " " + e.getArg2())
                                             })
		  ...
		  val kbpAllRelevantCandidates = substituteKBPRelations(allRelevantCandidates, firstQuery, topJobs, mapper)
          ...
          for(query <- sameQueriesRelevantSlot){		    
            ...
            println("query: " + querySetCount + " " + slots.toList(0).name)
    ...
    println("total doc count: " + totalDocCount)
       ...
       if(queryEntityType == "PER"){
         ...
         c.extr.getRel() match {  
           ...
           case _ =>   
	        ...
	        {e.printStackTrace()
	         println("EXCEPTION: Substitute Relations") 
	        }	  
    } 
