...
//class Candidate(val id: Int, val extr: Extraction, val types: List[Type]) {
class Candidate(val id: Int, val extr: KBPExtraction) {
  ...
  */
  ...
  lazy val entityOffsetInterval = Interval.closed(extr.getArg1().getStartOffset(),extr.getArg1().getEndOffset())
  ...
  */
  ...
  lazy val trimmedEntity = new TrimmedType(extr.getArg1().getArgName(), entityOffsetInterval) 
...
object Candidate {
  ...
  def groupScore(candidates: Candidates): Double = {
    ...
    1 - candidates.map(c => 1 - c.extr.getScore()).reduce(_ * _)
  }
