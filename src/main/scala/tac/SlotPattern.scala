...
object SlotPattern {
  ...
  def read(csvDataArray: Array[String]): Option[SlotPattern] = {
    ...
    csvDataArray match {
      case Array(slotName, maxValues, relString, arg2Begins, entityIn, slotFillIn, slotType, arg1Terms, arg2Terms, 
_*) => {
...
"")
      ...
      case _ => {
        System.err.println("Couldn't parse pattern fields(%d): %s".format(csvDataArray.length, 
csvDataArray.mkString(", ")))
        None
      }
    }
  }
